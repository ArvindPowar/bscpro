...
-(BOOL)checkSizeSanityAgainstOriginalImage:(UIImage *)originalImage {
	...
	NSLog(@"SANITY sizesMatch: '%d.'",TUYesOrNoString(sizesMatch));
	NSLog(@"SANITY topRow: '%d', midRow:'%d, lowRow:'%d'.", TUYesOrNoString(topRow),TUYesOrNoString(midRow),TUYesOrNoString(lowRow));
	NSLog(@"SANITY lCol: '%d', mCol:'%d, rCol:'%d'.", TUYesOrNoString(lCol),TUYesOrNoString(mCol),TUYesOrNoString(rCol));
	NSLog(@"SANITY <ow: '%.1f', tr: '%.1f', mr: '%.1f', lr: '%.1f'>",ow,tr,mr,lr);
	NSLog(@"SANITY <oh: '%.1f', lc: '%.1f', mc: '%.1f', lc: '%.1f'>",oh,lc,mc,rc);
	return sizesMatch;
}

#pragma mark TUNinePatch Overrides
-(void)drawInRect:(CGRect)rect {
	NPSelfProperty(_center);
	NPSelfProperty(_leftEdge);
	NPSelfProperty(_rightEdge);
	NPSelfProperty(_upperEdge);
	NPSelfProperty(_upperLeftCorner);
	NPSelfProperty(_upperRightCorner);
	NPSelfProperty(_lowerLeftCorner);
	NPSelfProperty(_lowerRightCorner);
